...
def unpack_lifx_message(packed_message):
    ...
    if message_type == MSG_IDS[GetService]:
        ...
        label = binascii.unhexlify("".join(["%2.2x" % (b & 0x000000ff) for b in struct.unpack("b"*32, payload_str[0:32])])).replace(b'\x00', b'').decode('utf-8')
        payload = {"label": label}
        ...
        label = binascii.unhexlify("".join(["%2.2x" % (b & 0x000000ff) for b in struct.unpack("b"*32, payload_str[16:48])])).replace(b'\x00', b'').decode('utf-8')
        ...
        label = binascii.unhexlify("".join(["%2.2x" % (b & 0x000000ff) for b in struct.unpack("b"*32, payload_str[12:44])])).replace(b'\x00', b'').decode('utf-8')
